...
export default function ProjectCard({ project, onEdit, onDelete, isAdmin = false }) {
  ...
  const handleMouseMove = (e) => {
    ...
    const rx = ((y / height) - 0.5) * -14;   // -7 .. +7 deg
    const ry = ((x / width)  - 0.5) *  14;
    card.style.transform = `perspective(900px) rotateX(${rx}deg) rotateY(${ry}deg) translateZ(6px)`;
    card.style.transition = 'transform 0.08s ease';

    if (glowRef.current) {
      glowRef.current.style.background = `radial-gradient(circle at ${x}px ${y}px, rgba(255,255,255,0.07), transparent 65%)`;
    }
  };
  ...
  // Prevent card click when interacting with buttons/links
      ...
      >
        <div
          ...
          style={{
            ...
            pointerEvents: 'none', zIndex: 5, transition: 'background 0.1s',
          }}
          aria-hidden="true"
        />
        ...
        <div className="project-card-body">
          ...
          <div className="project-card-links" onClick={stopPropagation}>
            {project.githubUrl && (
            ...
            {isAdmin && (
              <>
                <button onClick={() => onEdit(project)} className="btn btn-outline btn-sm">✏️ Edit</button>
                <button onClick={() => onDelete(project._id)} className="btn btn-danger btn-sm">🗑️ Delete</button>
              </>
            )}
          </div>
        </div>
